...
func main() {
	...
	userName := "test_DeleteUser"
	password := "test_DeleteUser"
	...
	// SignIn
	paramsSignIn := &cognitoidentityprovider.InitiateAuthInput{
	...
	respSignIn, errSignIn := svc.InitiateAuth(paramsSignIn)
	if errSignIn != nil {
		fmt.Println(errSignIn.Error())
		return
	}
	fmt.Println(respSignIn)
	fmt.Println("Completed")

	// SignIn
	AccessToken := respSignIn.AuthenticationResult.AccessToken
	paramsDeleteUser := &cognitoidentityprovider.DeleteUserInput{
		AccessToken: AccessToken,
	}

	respDeleteUser, errDeleteUser := svc.DeleteUser(paramsDeleteUser)
	if errDeleteUser != nil {
		fmt.Println(errDeleteUser.Error())
		return
	}
	fmt.Println(respDeleteUser)
	fmt.Println("Completed")
}
